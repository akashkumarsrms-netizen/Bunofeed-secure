...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.buildCardDropdownsHTML = function(product) {
    ...
    if (hasTextures) {
      ...
      html += `<div class="card-dropdown-placeholder" style="display:none;"></div>`;
    }
      ...
      html += `
        <div class="card-dropdown-group">
          <select class="card-variant-select card-size-select" data-id="${product.id}" aria-label="Select Weight">
            <option value="" disabled selected hidden>Pack Size</option>
    ...
    // Always include a Quantity dropdown on each product card
    html += `
      <div class="card-dropdown-group">
        <select class="card-variant-select card-qty-select" data-id="${product.id}" aria-label="Select Quantity">
    ...
    // Always include the "Buy Now" button in the remaining space of Row 2 Col 2
    html += `
      <div class="card-buy-btn-container">
        <button class="btn-buy buy-now-btn c-buy-btn" data-id="${product.id}">Buy Now</button>
      </div>
    `;
  ...
  window.BUNO_DIRECT_BUY = function(productId, chosenTexture, chosenSize, chosenQty) {
    ...
    grid.querySelectorAll('.product-card').forEach(card => {
      card.addEventListener('click', (e) => {
        if (!e.target.closest('.btn-buy') && !e.target.closest('.card-variant-select')) {
          const textureSel = card.querySelector('.card-texture-select');
          const sizeSel = card.querySelector('.card-size-select');
          openProductModal(card.dataset.id, textureSel ? textureSel.value : '', sizeSel && sizeSel.value ? sizeSel.value : '');
        }
      });
    });
    grid.querySelectorAll('.buy-now-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (product) {
          ...
          const qtySel = card ? card.querySelector('.card-qty-select') : null;
          
          const chosenTexture = textureSel ? textureSel.value : '';
          const chosenSize = sizeSel && sizeSel.value ? sizeSel.value : ((product.sizeVariants && product.sizeVariants.length > 0) ? product.sizeVariants[0] : (product.weight || ''));
          const chosenQty = qtySel && qtySel.value ? parseInt(qtySel.value) || 1 : 1;

          window.BUNO_DIRECT_BUY(product.id, chosenTexture, chosenSize, chosenQty);
        }
      });
    });
  ...
  window.updateCheckoutSummary = function() {
    ...
    // GST calculation
    let gstPercent = 18; // Fallback
    if (currentProduct.variantPrices) {
      let texture = "";
      let size = "";
      if (selectedVariantLabel) {
        let parts = selectedVariantLabel.split(',').map(s => s.trim());
        if (parts.length === 2) {
          size = parts[0];
          texture = parts[1];
        } else if (parts.length === 1) {
          let pVal = parts[0];
          if (currentProduct.sizeVariants && currentProduct.sizeVariants.includes(pVal)) {
            size = pVal;
          } else if (currentProduct.textureVariants && currentProduct.textureVariants.includes(pVal)) {
            texture = pVal;
          }
        }
      }
      
      if (!size && currentProduct.sizeVariants && currentProduct.sizeVariants.length > 0) {
        size = currentProduct.sizeVariants[0];
      }
      if (!texture && currentProduct.textureVariants && currentProduct.textureVariants.length > 0) {
        texture = currentProduct.textureVariants[0];
      }

      let combinationKey = (currentProduct.textureVariants && currentProduct.textureVariants.length > 0) 
        ? `${texture}-${size}` 
        : `default-${size}`;

      const varPriceInfo = currentProduct.variantPrices[combinationKey];
      if (varPriceInfo && varPriceInfo.gstPercent) {
        gstPercent = Number(varPriceInfo.gstPercent);
      }
    }

    const taxableBase = subtotal / (1 + gstPercent / 100);
    const gstAmount = subtotal - taxableBase;

    // Update summary UI elements
    const itemDetailsEl = document.getElementById('checkout-item-details');
    if (itemDetailsEl) {
      let suffix = selectedVariantLabel ? ` (${selectedVariantLabel})` : '';
      itemDetailsEl.textContent = `${currentProduct.name}${suffix} × ${qty}`;
    }

    const subtotalEl = document.getElementById('checkout-subtotal');
    const taxableBaseEl = document.getElementById('checkout-taxable-base');
    const gstLabelEl = document.getElementById('checkout-gst-label');
    ...
    if (taxableBaseEl) taxableBaseEl.textContent = `₹${taxableBase.toFixed(2)}`;
    if (gstLabelEl) gstLabelEl.textContent = `GST (${gstPercent}% Included):`;
